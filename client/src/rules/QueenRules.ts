...
export const queenMove = (initialPosition: Position, desiredPosition: Position, color: PieceColor, boardState: Piece[]): boolean => {
    for (let i = 1; i < 8; i++) {
        //Diagonal
...
export const getPossibleQueenMoves = (queen: Piece, boardstate: Piece[]): Position[] => {
    ...
    for (let i = 1; i < 8; i++) {
        ...
        } else if (tileIsOccupiedByOpponent(destination, boardstate, queen.color)) {
            possibleMoves.push(destination);
            break;
        } else {
            break;
