...
export const kingMove = (initialPosition: Position, desiredPosition: Position, color: PieceColor, boardState: Piece[]): boolean => {
    for (let i = 1; i < 2; i++) {
        //Diagonal
        ...
        if (passedPosition.isSimilar(desiredPosition)) {
            if (tileIsEmptyOrOccupiedByOpponent(passedPosition, boardState, color)) {
                return true;
            }
        } else {
...
export const getPossibleKingMoves = (king: Piece, boardstate: Piece[]): Position[] => {
    ...
    for (let i = 1; i < 2; i++) {
        ...
        } else if (tileIsOccupiedByOpponent(destination, boardstate, king.color)) {
            possibleMoves.push(destination);
            break;
        } else {
            break;
...
export const getCastlingMoves = (king: Piece, boardstate: Piece[]): Position[] => {
    ...
    for (const rook of rooks) {
        ...
        if (!rook.possibleMoves?.some(m => m.isSimilar(adjacentPosition))) continue;
        ...
        for (const enemy of enemyPieces) {
            ...
            for (const move of enemy.possibleMoves) {
                if (conceringTiles.some(t => t.isSimilar(move))) {
                    valid = false;
                }
